...
# ═══════════════════════════════════════════════════════
# UPLOAD IMAGES (с полным URL)
# ═══════════════════════════════════════════════════════

@app.post("/api/admin/upload", dependencies=[Depends(verify_admin)])
async def upload_files(request: Request, files: list[UploadFile] = File(...)):
    base_url = str(request.base_url).rstrip('/')  # например, https://твой-сайт.railway.app
    saved_urls = []
    for file in files:
...
def delete_brand(brand_id: int, db: Session = Depends(get_db)):
    ...
    if not brand:
        raise HTTPException(status_code=404, detail="Not found")
    db.query(models.Product).filter(models.Product.brand_id == brand_id).update(
        {"brand": "Not defined", "brand_id": None}
    )
    db.delete(brand)
    db.commit()
    return {"ok": True}
